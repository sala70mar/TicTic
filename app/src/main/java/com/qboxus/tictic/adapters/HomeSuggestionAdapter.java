...
public class HomeSuggestionAdapter extends RecyclerView.Adapter<HomeSuggestionAdapter.CustomViewHolder> {
    ...
    public void onBindViewHolder(final CustomViewHolder holder, final int i) {
        ...
        if (!(item.getGifLink().isEmpty()))
        {
            holder.userGif.setController(Functions.frescoImageLoad(item.getGifLink(),holder.userGif,true));
        }
        else
        {
            holder.userGif.setController(Functions.frescoImageLoad(item.getGifLink(),holder.userGif,false));
        }
