package com.qboxus.tictic.activitesfragments.argear.camera;
...
public class ReferenceCamera1 extends ReferenceCamera {
    ...
    private static final String TAG = "ARgear_";
    ...
    public ReferenceCamera1(Activity activity, CameraListener listener) {
    ...
    private Camera createCamera() {
        ...
        if (requestedCameraId == -1) {
            Functions.showToastOnTop(activity,null,"Could not find requested camera.");
        }
...
//            sizePair = selectSizePair(camera, requestedPreviewWidth, requestedPreviewHeight);
        ...
        if (sizePair == null) {
            Functions.showToastOnTop(activity,null,"Could not find suitable preview size.");
        }
        ...
        if (previewFpsRange == null) {
            Functions.showToastOnTop(activity,null,"Could not find suitable preview frames per second range.");
        }
    ...
    private class FrameProcessingRunnable implements Runnable {
        ...
        void release() {
           try {
               if (processingThread.getState()!=null)
               {
