...
public class GLView extends GLSurfaceView implements GLSurfaceView.Renderer {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int glviewWidth;
        int glviewHeight;
        if (getContext() instanceof VideoRecoderA)
        {
            glviewWidth = ((VideoRecoderA)getContext()).getGLViewWidth();
            glviewHeight = ((VideoRecoderA)getContext()).getGLViewHeight();
        }
        else
        {
            glviewWidth = ((VideoRecoderDuetA)getContext()).getGLViewWidth();
            glviewHeight = ((VideoRecoderDuetA)getContext()).getGLViewHeight();
        }
//        Log.d(Constants.tag, "onMeasure " + glviewWidth + " " + glviewHeight + " " + width + " " + height);
        ...
        if(glviewWidth > 0 && glviewHeight > 0) {
            ...
            if (getContext() instanceof VideoRecoderA)
            {
                ((VideoRecoderA)getContext()).setMeasureSurfaceView(this);
            }
            else
            {
                ((VideoRecoderDuetA)getContext()).setMeasureSurfaceView(this);
            }
